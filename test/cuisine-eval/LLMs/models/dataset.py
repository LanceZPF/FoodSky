...
class Yummly(Dataset):
    def __init__(self, dataset_dir, partition='test_Spanish'):
        self.dataset_dir = dataset_dir
        self.partition = partition
        
        # Load dataset
        data_path = os.path.join(dataset_dir, f"{partition}.json")
    ...
    def __getitem__(self, idx):
        ...
        if len(sample['images']) > 0:
            image_path = os.path.join(self.dataset_dir, sample['images'][0])
        else:
            image_path = ''
