...
# dir_name = 'newdata-type'
dir_name = 'newdata'

# def glm(modelpath, tokens):
#     tokenizer = AutoTokenizer.from_pretrained(modelpath, trust_remote_code=True)
#     model = AutoModel.from_pretrained(modelpath, trust_remote_code=True).half().cuda()
#     model = model.eval()
#     response, history = model.chat(tokenizer, tokens, history=[])
#     return response
...
def main(args, evaluator, take):
    ...
    filenames = os.listdir(f"{dir_name}/val")
    ...
    for index,subject_name in enumerate(subject_list):
        ...
        val_file_path=os.path.join(f'{dir_name}/val',f'{subject_name}_val.csv')
        dev_file_path=os.path.join(f'{dir_name}/dev',f'{subject_name}_dev.csv')
        test_file_path=os.path.join(f'{dir_name}/test',f'{subject_name}_test.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", default="/mnt/petrelfs/zhoupengfei/zpf/.cache/Qwen2.5-14B-Instruct", type=str)
    # parser.add_argument("--model_path", default="/mnt/petrelfs/zhoupengfei/zpf/FS/LLaMA-Factory/foodsky-7b-1215-5753",type=str)
